...
test('parseSupportedHttpUrl accepts only http/https with hostname', () => {
  assert.equal(parseSupportedHttpUrl('https://example.com/path').hostname, 'example.com')
  assert.equal(parseSupportedHttpUrl('http://example.com').hostname, 'example.com')
...
test('parseSupportedHostname returns hostname for supported URLs', () => {
  assert.equal(parseSupportedHostname('https://news.ycombinator.com/item?id=1'), 'news.ycombinator.com')
...
test('classifyPopupInjectionError maps cannot-access errors to user-friendly failure', () => {
  const result = classifyPopupInjectionError(new Error('Cannot access contents of url'))
...
test('classifyPopupInjectionError treats duplicate injection as success', () => {
  const result = classifyPopupInjectionError(new Error('Script already injected duplicate'))
  assert.deepEqual(result, {
    success: true,
    shouldLog: false,
  })
...
test('shouldLogBackgroundInjectionError ignores expected injection failures', () => {
  assert.equal(shouldLogBackgroundInjectionError(new Error('Cannot access page')), false)
  assert.equal(shouldLogBackgroundInjectionError(new Error('No tab with id: 1')), false)
  assert.equal(shouldLogBackgroundInjectionError(new Error('unexpected failure')), true)
})
