...
function updateAdvancedOptions(enabled, features) {
	...
	if (featureTextSelection)
		featureTextSelection.checked = features.textSelection;
...
async function setupAdvancedOptionsToggle() {
	...
	advancedToggle.addEventListener("click", () => {
		...
		chrome.storage.sync
			.set({ [UI_STATE_KEY]: { advancedExpanded: !isExpanded } })
			.catch(() => {});
	});
}
...
async function processToggleQueue() {
	...
	try {
		while (toggleQueue.length > 0) {
			const { tab, hostname, enabled, features, resolve, reject } =
				toggleQueue.shift();
