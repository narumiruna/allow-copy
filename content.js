...
(() => {
	...
	function createRightMouseEventHandler(eventType) {
		return (e) => {
			...
			const shouldPreventForEvent =
				eventType === "mousedown" || eventType === "mouseup";
			if (
				shouldPreventRightClickNavigation &&
				shouldPreventForEvent &&
				e.cancelable
			) {
				e.preventDefault();
			}
	...
	function detectRestrictions() {
		...
		elementsToCheck.forEach((element) => {
			...
			if (cssResults.pointerEvents)
				results.cssRestrictions.pointerEvents = true;
		...
		const addStyle = () => {
          ...
          `;
				...
				if (attempts < RAF_MAX_ATTEMPTS) {
					...
					console.log(
						"Allow Copy: document.head not available, skipping style injection",
					);
				}
			}
		};
		...
		DOCUMENT_PROPERTIES.forEach((prop) => {
			...
			try {
				const originalDescriptor =
					originalDocumentPropertyDescriptors.get(prop);
		...
		const startObserver = () => {
			if (document.head) {
				...
				if (attempts < RAF_MAX_ATTEMPTS) {
					...
					// Fallback: document.head not available after timeout, give up
					console.log(
						"Allow Copy: document.head not available, skipping observer setup",
					);
				}
			}
		};
	...
	function initialize(enabled, featureSettings = null) {
		...
		if (isEnabled) {
			...
			if (document.readyState === "loading" && !domReadyCleanupRegistered) {
				document.addEventListener("DOMContentLoaded", onDomReadyCleanup, {
					once: true,
				});
	...
	runWhenReady(async () => {
		...
		try {
			if (
				...
				typeof StorageUtils.getSiteConfig === "function"
			) {
	...
	function handlePossibleRightClickNavigation() {
		...
		if (
			performance.now() - lastRightClickAt >
			PREVENT_RIGHT_CLICK_NAV_WINDOW_MS
		)
			return;
